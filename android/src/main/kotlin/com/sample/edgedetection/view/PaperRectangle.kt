...
class PaperRectangle : View {
    ...
    init {
        ...
        circlePaint.strokeWidth = 6F              // thicker stroke for better visibility
    ...
    override fun onDraw(canvas: Canvas?) {
        ...
        // Draw corner circles – larger radius (30px) for easier touch
        if (cropMode) {
            canvas?.drawCircle(tl.x.toFloat(), tl.y.toFloat(), 30F, circlePaint)
            canvas?.drawCircle(tr.x.toFloat(), tr.y.toFloat(), 30F, circlePaint)
            canvas?.drawCircle(bl.x.toFloat(), bl.y.toFloat(), 30F, circlePaint)
            canvas?.drawCircle(br.x.toFloat(), br.y.toFloat(), 30F, circlePaint)
        }
    }
    ...
    /**
     * Find the closest corner to the touch point using Euclidean distance.
     * This gives a more accurate selection, especially with larger touch circles.
     */
    private fun calculatePoint2Move(downX: Float, downY: Float) {
        val points = listOf(tl, tr, br, bl)
        // Use squared distance to avoid sqrt
        point2Move = points.minByOrNull { corner ->
            val dx = corner.x - downX
            val dy = corner.y - downY
            dx * dx + dy * dy
        } ?: tl
    }
