...
class CropPresenter(
    ...
    /**
     * Use the original (uncropped) image as the final picture.
     * This bypasses the crop step and directly shows the original image
     * for further enhancement/rotation/saving.
     */
    fun useOriginal() {
        if (picture == null) {
            Log.i(TAG, "picture null?")
        ...
        // Convert original Mat to Bitmap
        val originalBitmap = Bitmap.createBitmap(picture.width(), picture.height(), Bitmap.Config.ARGB_8888)
        Utils.matToBitmap(picture, originalBitmap)

        // Set as the cropped picture so enhancements/rotations work on it
        croppedPicture = picture
        croppedBitmap = originalBitmap
        rotateBitmap = originalBitmap
        enhancedPicture = originalBitmap

        // Show it in the UI and hide the overlay
        iCropView.getCroppedPaper().setImageBitmap(originalBitmap)
        iCropView.getPaper().visibility = View.GONE
        iCropView.getPaperRect().visibility = View.GONE
    }
